...
- **Markdown reading view** 📖 — every note opens either in the editor or as rendered Markdown, in its own panel. GitHub-flavoured, so tables, fenced code, task lists and strikethrough all work. It follows the font and size toggles like the editor does, keeping code monospace regardless. The mode is remembered per note, and switched with the Edit / Markdown control above the editor.
...
- **Width, font & size toggles** — narrow/wide, sans/mono, normal/larger. "Larger" scales the root font size, so every `rem` in the app grows together instead of just the editor; borders and other pixel values stay put, because a 1px rule does not want to be 1.085px.
